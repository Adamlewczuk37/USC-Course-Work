...
def save_mail(mail: List[Dict[str, str]]) -> None:
    """
    Saves mail as list of dictionaries in server
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    """
    Runs entire process of adding mail to server with help of previous 2 methods

    Returns:
        string: A string representing the unique id given for the mail entry
    """
    mail = load_mail()
...
def delete_mail(mail_id: str) -> bool:
    """
    Deletes all mail elements with specified ID after loading it and saves new 
    state of stored mail

    Returns:
        bool: True is mail was successfully deleted; False otherwise
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    """
    Gets all mail elements with specified ID after loading it

    Returns (optional):
        Directory: Dictionary representing a mail entry
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """
    Get all mail stored in inbox held in server

    Returns:
        list: A list of dictionaries representing the mail entries stored in inbox
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """
    Get all mail stored in sent messages held in server

    Returns:
        list: A list of dictionaries representing the mail entries stored in sent
    """
    mail = load_mail()
    sent = []
    for entry in mail:
