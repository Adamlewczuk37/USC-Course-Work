...
protected:
    ...
    virtual Node<Key, Value>* internalFind(const Key& k) const;
    ...
    bool balancedHelper(Node<Key,Value>* n) const;
...
{
    Node<Key, Value>* equ = internalFind(keyValuePair.first);
    if (root_ == NULL){
        ...
        Node<Key, Value>* parent = root_;
				while ((parent->getKey() != keyValuePair.first) && (parent != NULL)){
        ...
        equ->setValue(keyValuePair.second);
    }
}
...
void BinarySearchTree<Key, Value>::remove(const Key& key)
{
    if (root_ == NULL){
		...
		Node<Key, Value>* equ = internalFind(key);
		if (equ == NULL){
			return;
		}

		Node<Key, Value>* parent = equ->getParent();
		Node<Key, Value>* pred = predecessor(equ);
		Node<Key, Value>* lef = equ->getLeft();
		Node<Key, Value>* rig = equ->getRight();
		...
		} else if ((equ->getLeft() == NULL) || (equ->getRight() == NULL)){
			if (equ->getLeft() != NULL){
...
bool BinarySearchTree<Key, Value>::balancedHelper(Node<Key, Value>* current) const
{
   ...
   int right = depth(current->getRight());
   int left = depth(current->getLeft());
   if ((right == 0) && (left == 0)){
       return true;
   }
   right -= left;
   if ((right < 2) && (right > -2)){
       return (balancedHelper(current->getRight()) && balancedHelper(current->getLeft()));
   } else {
       return false;
   }
}
...
template<typename Key, typename Value>
int BinarySearchTree<Key, Value>::depth(Node<Key, Value>* current) const
{
   if (current == NULL){
       return 0;
   }
   int left = depth(current->getLeft()) + 1;
   int right = depth(current->getRight()) + 1;
   if (left > right){
       return left;
   } else {
       return right;
